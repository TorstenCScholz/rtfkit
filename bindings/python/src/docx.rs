...
use rtfkit_docx::{write_docx, write_docx_to_bytes, DocxWriterOptions};
use std::path::Path;
...
pub fn py_to_docx_bytes<'py>(py: Python<'py>, document: &PyDocument) -> PyResult<Bound<'py, PyBytes>> {
    let bytes = write_docx_to_bytes(&document.inner, &DocxWriterOptions::default())
...
pub fn py_to_docx_file(document: &PyDocument, path: &str) -> PyResult<()> {
    write_docx(&document.inner, Path::new(path), &DocxWriterOptions::default())
        .map_err(|e| errors::DocxWriterError::new_err(e.to_string()))
}
