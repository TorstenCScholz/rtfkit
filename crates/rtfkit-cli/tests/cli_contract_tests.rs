...
// =============================================================================
// IMAGE CLI CONTRACT TESTS
// =============================================================================
//
// Tests for image support (from PLAN_IMAGES.md sections 6 and 8):
// - PNG/JPEG images convert successfully to DOCX/HTML/PDF
// - Multiple images in one document
// - IR output contains ImageBlock with correct data
// - Strict mode behavior for valid and invalid images
// - Warning messages for unsupported/malformed images
...
mod image_cli_tests {
    ...
    fn image_png_output_is_deterministic() {
        ...
        let mut parsed1: serde_json::Value =
            serde_json::from_str(&json1).expect("JSON should parse");
        let mut parsed2: serde_json::Value =
            serde_json::from_str(&json2).expect("JSON should parse");
...
// =============================================================================
// Document Structure Tests (Headers / Footers / Footnotes / Endnotes)
// =============================================================================
            ...
            b.get("inlines")
                ...
                .map(|inlines| {
                    inlines.iter().any(|inline| {
                        inline.get("type").and_then(|t| t.as_str()) == Some("note_ref")
                    })
                })
                .unwrap_or(false)
        });
